...
function_collect_data(){
	if [[ -d "${falcon_installation_path}" ]]; then
		falcon_customerID="$(/usr/bin/sudo /Applications/Falcon.app/Contents/Resources/falconctl stats agent_info | /usr/bin/grep -i "customerID:" | /usr/bin/awk '{print $2}')"
		echo "<result>${falcon_customerID}</result>"
	else
